...
weissensee$nTime <- difftime(strptime(weissensee$Time,"%H:%M:%S"),
                     strptime("00:00:00","%H:%M:%S"), units="secs")
weissensee.means <- aggregate(weissensee$nTime, by=list(weissensee$Year), mean)
weissensee.means$Time <- format(.POSIXct(weissensee.means$x,tz="GMT"), "%H:%M:%S")
names(weissensee.means) <- c("Year", "x", "Average finish time")
weissensee.means[, c(1,3)]
```
